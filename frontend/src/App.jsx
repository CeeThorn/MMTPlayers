...
const addPlayer = async (input) => {
  ...
  if (players.length >= 12) {
    alert("You’ve reached the 12-player limit. Please delete one to add another.");
    return; 
  }
 ...
 const detectedType = detectMediaTypeImport(url);
  const finalType = providedType || detectedType || "unsupported";
   const channel = await getChannelName(url, finalType);

   console.log("[addPlayer] Channel name resolved:", channel);

  const newPlayer = {
    id: Date.now(),
    url,
    name: name || "Player" || url,
    volume: 1,
    channel,
    type: finalType,
    ref: null,
...
// Update ref for a player (pass from MediaPlayer)
  const updatePlayerRef = (id, ref) => {
    // 1. Update the mutable ref object (for immediate access)
    playerRefs.current[id] = ref;

    // 2. Update the state ONLY IF the ref is not already set on the player object
    setPlayers(prev => {
        const playerToUpdate = prev.find(p => p.id === id);
        
        // Check if the player exists AND if the ref value is currently null/unset
        // If the ref is ALREADY set on the state object, we don't need to re-render.
        if (playerToUpdate && playerToUpdate.ref !== ref) { 
            return prev.map(player => player.id === id ? { ...player, ref } : player);
        }
        
        // No update needed (ref was already set or player not found)
        return prev; 
    });
};
